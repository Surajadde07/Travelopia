...
const Religious = () => {
    ...
    useEffect(() => {
        const fetchTours = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/routes/religious`);
            ...
            <div className="px-6 py-12">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
                    {tours.map((tour, index) => (
                        <div
                            key={index}
                            className="bg-white rounded-xl shadow-xl transform hover:scale-105 hover:shadow-2xl transition-all duration-300 w-[340px] sm:w-[380px] lg:w-[420px] relative"
                        >
                            {/* Image Section */}

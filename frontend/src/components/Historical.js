        ...
        const fetchTours = async () => {
            try {
                const response = await axios.get(
                    `${process.env.REACT_APP_BACKEND_URL}/api/user/dashboard/tours/historical-cultural`,
                    {
            ...
            <div className="px-6 py-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                    {tours.map((tour, index) => (
                        <div key={index} className="max-w-[520px] bg-white rounded-lg shadow-lg overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-2xl">
                            {/* Image Section */}
                            ...
                            <div className="p-6 space-y-4">
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <Link to={`/dashboard/booknow/${tour._id}`} className="bg-[#A0522D] text-white px-4 py-2 rounded-md text-sm font-semibold hover:bg-[#8B4513] transition-all duration-300">
                                        Book Now →
                                    </Link>
                                </div>
                            </div>
